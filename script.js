...
class NepaliCalendar {
    ...
    generateCalendar() {
        ...
        const firstDayOfWeek = 4; // Starting day of week for current month (0 = Sunday)
        
        // Add empty cells for days before month starts
        for (let i = 0; i < firstDayOfWeek; i++) {
            const emptyDay = document.createElement('div');
            emptyDay.className = 'calendar-day other-month';
            const prevMonthDay = 30 - (firstDayOfWeek - i - 1);
            emptyDay.innerHTML = `
                <div class="day-number">${prevMonthDay}</div>
                <div class="day-nepali">${this.toNepaliNumber(prevMonthDay)}</div>
            `;
        ...
        for (let i = 1; i <= remainingCells; i++) {
            ...
            emptyDay.innerHTML = `
                ...
                <div class="day-nepali">${this.toNepaliNumber(i)}</div>
            `;
            calendarDays.appendChild(emptyDay);
        }
